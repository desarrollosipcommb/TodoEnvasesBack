...
public class QuimicosService {
    ...
    public QuimicosDTO updateInventoryQuimico(String nameJar, Integer quantityQuimico, String token) {
        ...
        if (quantityQuimico < 0) {
            quimico.setQuantity(Math.max((quimico.getQuantity() + quantityQuimico), 0));
            inventoryService.newItem(quimico.getId().longValue(), "quimico", quimico.getQuantity(),
                    "damage", jwtService.getUserIdFromToken(token).intValue(), "Se reporto un daño en "
                            + quimico.getName() + " su inventario ahora es: " + quimico.getQuantity());
            return new QuimicosDTO(quimicosRepository.save(quimico));
        }
        quimico.setQuantity(quantityQuimico);
        inventoryService.newItem(quimico.getId().longValue(), "quimico", quimico.getQuantity(), "restock",
                jwtService.getUserIdFromToken(token).intValue(), "Se actualizo " + quimico.getName()
                        + " su inventario ahora es: " + quimico.getQuantity());
