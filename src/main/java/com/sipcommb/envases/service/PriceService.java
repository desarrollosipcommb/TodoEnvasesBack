...
public class PriceService {
    ...
    public boolean verifyPriceSearchRequest(PriceSearchRequest priceSearchRequest) {
        if (priceSearchRequest.getExactPrice() == null && priceSearchRequest.getMinPrice() == null
                && priceSearchRequest.getMaxPrice() == null) {
        ...
        if (priceSearchRequest.getMinPrice() != null
                && priceSearchRequest.getMinPrice().compareTo(BigDecimal.ZERO) < 0) {
            throw new IllegalArgumentException("El precio mínimo no puede ser negativo.");
        }

        if (priceSearchRequest.getMaxPrice() != null
                && priceSearchRequest.getMaxPrice().compareTo(BigDecimal.ZERO) < 0) {
            throw new IllegalArgumentException("El precio máximo no puede ser negativo.");
        }

        if (priceSearchRequest.getExactPrice() != null
                && priceSearchRequest.getExactPrice().compareTo(BigDecimal.ZERO) < 0) {
            throw new IllegalArgumentException("El precio exacto no puede ser negativo.");
        }

        if (priceSearchRequest.getMaxPrice() != null && priceSearchRequest.getMinPrice() != null
                && priceSearchRequest.getMaxPrice().compareTo(priceSearchRequest.getMinPrice()) < 0) {
